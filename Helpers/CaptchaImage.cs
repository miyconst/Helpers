...
namespace Helpers
{
    ...
    public class CaptchaImage
    {
        ...
        static CaptchaImage()
        {
            ...
            TextLength = 5;
            TextChars = "012346789";//ACDEFGHJKLNPQRTUVXYZ
            CacheTimeOut = 180D;
        }
        ...
        private GraphicsPath TextPath(string s, Font f, Rectangle r)
        {
            ...
            gp.AddString(s, f.FontFamily, (int)f.Style, fsize, r, sf);
            while (gp.PathPoints.Any(p => p.X > this.Width) && fsize > 10)
            {
                fsize--;
                gp = new GraphicsPath();
                gp.AddString(s, f.FontFamily, (int)f.Style, fsize, r, sf);
        ...
        /// <summary>
        /// Корректно введенное значение капчи в данном контексте
        /// </summary>
        /// <param name="context"></param>
        /// <returns></returns>
        public static bool IsCorrectlyCapcha(HttpContextBase context)
        {
            string guid = context.Request.Form["captcha-guid"];
            CaptchaImage image = CaptchaImage.GetCachedCaptcha(guid);
            string actualValue = context.Request.Form["captcha"];
            string expectedValue = image == null ? String.Empty : image.Text;

            var isCorrectly = !(String.IsNullOrEmpty(actualValue) || String.IsNullOrEmpty(expectedValue) || !String.Equals(actualValue, expectedValue, StringComparison.OrdinalIgnoreCase));
            return isCorrectly;
        }
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        public static bool IsCorrectlyCapcha(HttpContext context)
        {
            string guid = context.Request.Form["captcha-guid"];
            CaptchaImage image = CaptchaImage.GetCachedCaptcha(guid);
            string actualValue = context.Request.Form["captcha"];
